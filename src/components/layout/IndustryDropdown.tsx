...
import { industryNavItems, industryOverview } from "@/lib/industryNav";
...
export default function IndustryDropdown({
  ...
  const OverviewIcon = industryOverview.icon;

  if (variant === "mobile") {
    return (
      <div className="space-y-2 px-1 pb-2">
        <Link
          href={industryOverview.href}
          onClick={onClose}
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-bold text-razo-dark">{industryOverview.label}</p>
            <p className="text-xs text-razo-gray">{industryOverview.description}</p>
          </div>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-razo-gray">{item.description}</p>
                </div>
              </Link>
            );
          })}
  ...
  return (
    <motion.div
      ...
      className="absolute left-1/2 top-full z-40 mt-2 w-[min(400px,calc(100vw-32px))] -translate-x-1/2"
    >
            ...
            return (
              ...
              >
                <Link
                  href={item.href}
                  onClick={onClose}
                  className="group flex items-center gap-3 rounded-xl p-3 transition-all hover:bg-razo-blue/[0.05]"
                >
