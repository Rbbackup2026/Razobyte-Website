...
import {
  ...
  smmWhyChoose,
  smmServices,
  smmPlatformComparison,
  smmPackages,
  smmPackageComparison,
  smmTechStack,
  smmProcess,
  smmTrustBadges,
  smmTestimonials,
...
function SmmCampaignMockup() {
  const campaigns = [
    { name: "IG Reels — Brand", imp: "84K", eng: "7.2%", color: "#E4405F" },
    { name: "FB Lead Ads", imp: "52K", eng: "4.8%", color: "#1877F2" },
    { name: "LinkedIn Sponsored", imp: "18K", eng: "3.1%", color: "#0A66C2" },
  ];

  const orbit = [
    { label: "Reach", icon: "📡", x: -115, y: -48, color: SMM_ORANGE },
    { label: "Engage", icon: "💬", x: 115, y: -45, color: SMM_TEAL },
    { label: "Leads", icon: "🎯", x: -108, y: 60, color: "#DB2777" },
    { label: "ROI", icon: "💰", x: 110, y: 55, color: "#059669" },
  ];

  return (
    <div className="relative mx-auto flex h-[420px] w-full max-w-md items-center justify-center sm:h-[460px]">
      <div className="absolute h-60 w-60 rounded-full border border-dashed sm:h-72 sm:w-72" style={{ borderColor: `${SMM_ORANGE}44` }} />
      <div className="absolute h-84 w-84 rounded-full border" style={{ borderColor: `${SMM_TEAL}22` }} />
      ...
      <motion.div
        ...
        style={{ boxShadow: `0 32px 80px ${SMM_ORANGE}44` }}
      >
        <div className="flex items-center justify-between px-4 py-2.5 text-white" style={{ background: `linear-gradient(90deg, ${SMM_ORANGE}, ${SMM_TEAL})` }}>
          <span className="flex items-center gap-1.5 text-[10px] font-bold">
            <Megaphone size={14} /> SMM CAMPAIGNS
          </span>
        ...
        <div className="bg-slate-900 p-3">
          <div className="mb-2 flex items-center justify-between rounded-lg bg-slate-800 px-3 py-2">
            <span className="text-[9px] font-bold text-orange-400">● CAMPAIGNS LIVE</span>
            <span className="text-[9px] text-slate-400">budget ₹68K/mo</span>
          </div>
            ...
            {campaigns.map((c) => (
              <div key={c.name} className="mb-1 flex items-center justify-between rounded bg-slate-700/60 px-2 py-1">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-[8px] text-slate-400">{c.imp}</span>
                  <span className="text-[9px] font-bold" style={{ color: c.color }}>{c.eng}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {[
              { label: "Reach", val: "154K", color: SMM_ORANGE },
              { label: "Leads", val: "328", color: SMM_TEAL },
              { label: "ROAS", val: "4.1x", color: "#059669" },
            ].map((m) => (
      ...
      <motion.div
        ...
        style={{ background: SMM_TEAL }}
      >
        <p className="text-[9px] font-bold text-white/80">BRAND</p>
        <p className="text-xs font-bold text-white">UNSTOPPABLE 🚀</p>
      </motion.div>
      ...
      <div className="orb absolute -left-6 bottom-6 h-32 w-32" style={{ background: `${SMM_ORANGE}20` }} />
      <div className="orb absolute right-0 top-8 h-28 w-28" style={{ background: `${SMM_TEAL}18` }} />
    </div>
  );
}

export default function SmmSocialMediaMarketingPageContent() {
  const stats = getSmmStats();

  return (
    <main className="bg-white">
      <section className="relative overflow-hidden border-b border-gray-100 pt-28" style={{ background: "linear-gradient(160deg, #fff7ed, #f0fdfa 45%, white)" }}>
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:py-16">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <span className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-widest" style={{ borderColor: `${SMM_ORANGE}44`, background: `${SMM_ORANGE}10`, color: SMM_ORANGE }}>
              <Megaphone size={12} /> SMM Services
            </span>
            <h1 className="mt-4 text-3xl font-bold leading-tight text-razo-dark sm:text-4xl lg:text-[2.65rem]">{smmHero.title}</h1>
            <p className="mt-3 text-lg font-semibold" style={{ color: SMM_TEAL }}>{smmHero.subtitle}</p>
            <p className="mt-4 text-sm leading-relaxed text-razo-gray sm:text-base">{smmHero.description}</p>
            <ul className="mt-6 grid gap-2 sm:grid-cols-2">
              {smmHighlights.map((item) => (
                <li key={item} className="flex items-center gap-2 text-sm text-razo-gray">
                  <CheckCircle2 size={16} style={{ color: SMM_ORANGE }} className="shrink-0" /> {item}
                </li>
              ))}
            </ul>
      ...
      <section className="border-b border-gray-100" style={{ background: `linear-gradient(90deg, ${SMM_ORANGE}08, ${SMM_TEAL}08)` }}>
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-3 px-4 py-8 sm:px-6 lg:grid-cols-4">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
              ...
              <p className="text-2xl font-bold sm:text-3xl" style={{ color: SMM_ORANGE }}>{stat.value}</p>
      ...
      <section className="border-b border-gray-100 py-6">
        <div className="smm-scroll mx-auto flex max-w-5xl gap-4 overflow-x-auto px-4 pb-2">
      ...
      <section className="py-14 lg:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-3 text-center text-2xl font-bold text-razo-dark sm:text-3xl">Social Media Marketing That Connects, Engages, and Grows</h2>
          <p className="mx-auto mb-10 max-w-3xl text-center text-sm leading-relaxed text-razo-gray">{smmIntro}</p>
          <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-2">
            <div className="tilt-card overflow-hidden rounded-3xl border border-gray-100 shadow-xl">
              <Image src="/images/service-marketing.jpg" alt="Social media marketing" width={600} height={420} className="h-64 w-full object-cover sm:h-72" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              {smmWhyImportant.map((item, i) => {
                const Icon = item.icon;
                return (
                  <motion.div key={item.title} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                    className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                    <span className="flex h-14 w-14 items-center justify-center rounded-xl" style={{ background: `${SMM_ORANGE}12` }}>
                      <Icon size={26} style={{ color: SMM_ORANGE }} />
                    </span>
      ...
      <section className="py-14" style={{ background: `linear-gradient(135deg, ${SMM_ORANGE}, #ea580c)` }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-3 text-center text-2xl font-bold text-white sm:text-3xl">What We Offer</h2>
          <p className="mx-auto mb-10 max-w-2xl text-center text-sm text-white/80">Eight pillars of expert Social Media Marketing.</p>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {smmOfferings.map((item, i) => {
              const Icon = item.icon;
              return (
                <motion.div key={item.title} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                  className="tilt-card rounded-2xl bg-white p-6 shadow-xl">
                  <span className="flex h-16 w-16 items-center justify-center rounded-2xl text-white shadow-md" style={{ background: `linear-gradient(135deg, ${SMM_ORANGE}, ${SMM_TEAL})` }}>
                    <Icon size={30} />
                  </span>
      ...
      <section className="bg-razo-surface py-14">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <h2 className="mb-8 text-center text-2xl font-bold text-razo-dark">Case Study: Success Through Social Media</h2>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
            className="tilt-card overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-xl">
            <div className="grid lg:grid-cols-2">
              <div className="relative h-48 lg:h-auto">
                <Image src="/images/portfolio-ecommerce.jpg" alt="Case study" fill className="object-cover" sizes="400px" />
              </div>
              <div className="p-8">
                <div className="space-y-4">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: SMM_ORANGE }}>Client</p>
                    <p className="mt-1 text-sm text-razo-dark">{smmCaseStudy.client}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: SMM_TEAL }}>Challenge</p>
                    <p className="mt-1 text-sm text-razo-gray">{smmCaseStudy.challenge}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-razo-blue">Our Approach</p>
                    <p className="mt-1 text-sm text-razo-gray">{smmCaseStudy.approach}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600">Results</p>
                    <ul className="mt-2 space-y-1">
                      {smmCaseStudy.results.map((r) => (
      ...
      <section className="py-14">
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed text-razo-gray">{smmWhyChoose}</p>
            <Link href={homeRoutes.contact} className="mt-6 inline-flex items-center gap-2 rounded-full px-6 py-2.5 text-sm font-semibold text-white" style={{ background: SMM_TEAL }}>
      ...
      <section id="packages" className="scroll-mt-28 py-14" style={{ background: `linear-gradient(135deg, ${SMM_TEAL}, #0f766e)` }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-3 text-center text-2xl font-bold text-white sm:text-3xl">SMM Management Packages</h2>
          <p className="mx-auto mb-10 max-w-2xl text-center text-sm text-white/80">Scalable social media marketing from startups to enterprise brands.</p>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {smmPackages.map((plan, i) => (
              <motion.div key={plan.id} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                className={`relative rounded-2xl border bg-white p-6 shadow-lg transition hover:-translate-y-1 ${plan.popular ? "border-orange-300 ring-2 ring-orange-200" : "border-gray-100"}`}>
                {plan.popular && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full px-3 py-0.5 text-[10px] font-bold text-white" style={{ background: SMM_ORANGE }}>Most Popular</span>
                )}
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] border-collapse text-left text-sm">
                <thead>
                  <tr style={{ background: `linear-gradient(90deg, ${SMM_ORANGE}, ${SMM_TEAL})` }}>
                ...
                <tbody>
                  {smmPackageComparison.map((row, ri) => (
      ...
      <section className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-3 text-center text-2xl font-bold text-razo-dark">Platform SMM Comparison</h2>
          <p className="mx-auto mb-8 max-w-2xl text-center text-sm text-razo-gray">Facebook vs Instagram vs LinkedIn vs YouTube — marketing capabilities at a glance.</p>
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px] border-collapse text-left text-sm">
                ...
                <tbody>
                  {smmPlatformComparison.map((row, ri) => (
                    <tr key={row.feature} className={ri % 2 === 0 ? "bg-teal-50/30" : "bg-white"}>
                      <td className="border-t border-gray-100 px-4 py-3 font-medium text-razo-dark">{row.feature}</td>
      ...
      <section className="bg-razo-surface py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-10 text-center text-2xl font-bold text-razo-dark">SMM Services We Offer</h2>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {smmServices.map((svc, i) => {
              const Icon = svc.icon;
              return (
                <motion.div key={svc.title} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                  ...
                  <span className="flex h-14 w-14 items-center justify-center rounded-xl" style={{ background: `${SMM_ORANGE}12` }}>
                    <Icon size={28} style={{ color: SMM_ORANGE }} />
                  </span>
      ...
      <section className="py-14" style={{ background: `linear-gradient(135deg, ${SMM_ORANGE}, ${SMM_TEAL})` }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-3 text-center text-2xl font-bold text-white">SMM Tools & Platforms</h2>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
            {smmTechStack.map((t, i) => (
              <motion.div key={t.name} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                ...
                <span className="mt-1 text-xs font-semibold" style={{ color: SMM_ORANGE }}>{t.category}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-razo-surface py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-10 text-center text-2xl font-bold text-razo-dark">Our SMM Process</h2>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {smmProcess.map((p, i) => (
              <motion.div key={p.step} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                ...
                <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full text-sm font-bold text-white" style={{ background: i % 2 === 0 ? SMM_ORANGE : SMM_TEAL }}>
                  {i + 1}
                </div>
      ...
      <section className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <h2 className="mb-8 text-center text-2xl font-bold text-razo-dark">SMM Success Stories</h2>
            ...
            {smmProjects.map((p, i) => (
              <motion.div key={p.id} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                ...
                <div className="relative h-48">
                  ...
                  <span className="absolute left-3 top-3 rounded-full px-3 py-1 text-[11px] font-bold text-white" style={{ background: SMM_ORANGE }}>{p.tag}</span>
                </div>
                <div className="p-5">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {smmTestimonials.map((t, i) => (
              <motion.div key={t.name} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                ...
                <Quote size={28} className="absolute right-5 top-5 opacity-10" style={{ color: SMM_ORANGE }} />
      ...
      <section className="py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="mb-8 text-center">
            ...
            <p className="mt-1 text-sm text-razo-gray">SMM tips, ad strategies, and social growth guides.</p>
          </div>
            ...
            {smmBlogs.map((post, i) => (
              <motion.article key={post.title} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                ...
                <div className="p-5">
                  ...
                  <Link href={post.href} className="mt-3 inline-flex items-center gap-1 text-xs font-semibold" style={{ color: SMM_ORANGE }}>Read more <ArrowRight size={12} /></Link>
                </div>
              </motion.article>
            ))}
          </div>
      ...
      <ProductFAQ faqs={smmFaqs} title="Frequently Asked Questions" description="Everything about SMM, paid social, platforms, and campaign strategies." variant="light" />

      <section className="border-t py-12">
        <div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-6 px-4 sm:flex-row sm:px-6">
          <div className="flex items-center gap-4">
            <span className="flex h-12 w-12 items-center justify-center rounded-xl text-white" style={{ background: `linear-gradient(135deg, ${SMM_ORANGE}, ${SMM_TEAL})` }}>
              <Megaphone size={24} />
            </span>
            <div>
              <p className="text-lg font-bold text-razo-dark sm:text-xl">Ready to make your brand the talk of the feed?</p>
              <p className="mt-1 text-sm text-razo-gray">Expert SMM campaigns from Razobyte&apos;s certified team.</p>
            </div>
          </div>
